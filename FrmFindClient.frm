...
Begin VB.Form FrmFindClient 
   ...
   Begin VB.Frame Frame1 
      Caption         =   "Search Result (Double Click the Required Client Information to go Back)"
      Height          =   2310
      Left            =   60
      TabIndex        =   4
      Top             =   1605
   ...
   End
End
Attribute VB_Name = "FrmFindClient"
...
'On Error Resume Next
    ...
    End If
    'concat conditions
        If Picture1(0).BackColor = vbGreen Then SqlStr1 = SqlStr1 & "clientRefNo = " & Val(Combo3.Text)
        ...
        End If
SqlStr1 = "Select * from clientmaster where " & SqlStr1
...
Private Sub Form_Load()
    ...
    RsTemp.Open "select clientrefno from clientmaster group by clientrefno", db, adOpenStatic, adLockReadOnly
    ...
    If RsTemp.State = 1 Then RsTemp.Close
    RsTemp.Open "select sname from clientmaster group by sname", db, adOpenStatic, adLockReadOnly
    If RsTemp.RecordCount > 0 Then
        RsTemp.MoveLast
        trec = RsTemp.RecordCount
        For i = 0 To trec - 1
    ...
    If RsTemp.State = 1 Then RsTemp.Close
    RsTemp.Open "select fname from clientmaster group by fname", db, adOpenStatic, adLockReadOnly
    ...
    If RsTemp.State = 1 Then RsTemp.Close
    RsTemp.Open "select surname from clientmaster group by surname"
